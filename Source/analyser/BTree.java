package analyser;
/** Balanced Tree Implementation class which extends AbstractSet and implements Set */
...
//	public static ResourceBundle fileBundle = ResourceBundle.getBundle("fileBundle");
	...
	public void updateDictTree(String fileName)
	{
		...
		BufferedReader dic = null;
			try
			{
				//String s = FileLocator.getFile(fileName).getPath();


				dic = new BufferedReader(new InputStreamReader(getClass().getResourceAsStream(fileName)));
				//System.out.println("loads bond"+fileName);

				//dic = new FileReader(s);
	...
	private class BTreeNode
	{
		...
		private int childToInsertAt(Object Obj, boolean position)
		{
			...
			while (min <= max)
			{
				...
				//System.out.println(index + " " + Obj1 + " &&&&& " + Obj2);
				if (BTree.this.compare(Obj1, Obj2) > 0)
					min = index + 1;
				else
					max = index - 1;
...
//			System.out.println(Obj + " inserted @ " + insertAt + " " + ndCount);
			nrElements++;
		}
	}
}